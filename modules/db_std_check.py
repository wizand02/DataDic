...
TARGET_TABLE_SCHEMAS = {
    ...
    "공통표준단어": ["번호", "제정차수", "공통표준단어명", "공통표준단어영문약어명", "공통표준단어 영문명", "공통표준단어 설명", "형식단어\\n여부", "공통표준도메인분류명", "이음동의어 목록", "금칙어 \\n목록"],
...
def show_db_std_check():
    ...
    with tabs[idx_load]:
        with st.container():
            ...
            if uploaded_file:
                if st.button("파일업로드 및 원본데이터적재", use_container_width=True, key="btn_upload_ori_std"):
                    overlay_placeholder.markdown("""
                    <div class='std-loading-overlay'>
                      <div class='std-loading-box'>
                        <div class='std-loading-spinner'>⚙️</div>
                        <div class='std-loading-text'>데이터 적재 중...</div>
                    ...
                    if not is_expert:
                        ...
                        if success:
                            ...
                            st.error("⚠️ 일부 시트 처리 중 오류가 발생했습니다. 위 에러 메시지를 확인하세요.")
                    else:
                        overlay_placeholder.empty()
                        st.toast(f"✅ {len(loaded)}개 시트 ORI 테이블 적재 완료!", icon="✅")
                        st.rerun()

                loaded_sheets = st.session_state.get('std_check_loaded_sheets', [])

                if loaded_sheets and is_expert:
                    if 'std_check_mapped_status' not in st.session_state:
                        st.session_state['std_check_mapped_status'] = {s: False for s in loaded_sheets}
                    ...
                    if st.button("🚀 전체 워크시트 자동 매핑 및 저장", type="primary", use_container_width=True, key="btn_auto_map_all_std"):
                        with st.spinner("전체 워크시트를 자동 매핑하여 저장합니다..."):
                            success = auto_map_and_save_std(db_path, loaded_sheets)
                        if success:
                            st.toast("✅ 전체 워크시트 자동 매핑 및 저장 완료!", icon="✅")
                            import time; time.sleep(1.5)
                            st.rerun()
                        else:
                            st.error("⚠️ 전체 매핑 중 일부 오류가 발생했습니다.")

                    st.write("")
                    
                    for idx, sheet in enumerate(loaded_sheets):
                        ori_tb_name = f"ORI_{sheet}"
                        is_mapped = st.session_state['std_check_mapped_status'].get(sheet, False)
                        ...
                        with st.expander(expander_title, expanded=False):
                            ...
                            if st.button("저장 실행", key=f"btn_save_{idx}"):
                                col_defs = [f'"{col}" VARCHAR(500)' for col in target_columns]
                                create_sql = f"DROP TABLE IF EXISTS {selected_target};\nCREATE TABLE {selected_target} (\n  " + ",\n  ".join(col_defs) + "\n);"
                                ...
                                st.session_state['std_check_mapped_status'][sheet] = True
                                drop_std_check_results(db_path)
                                st.toast(f"✅ `{selected_target}` 테이블 적재 완료!")
                                st.rerun()
